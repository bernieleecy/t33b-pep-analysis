...
def make_df(file, start_res=1, end_res=20):
    ...
    melt_df = pd.melt(df, id_vars=['Peptide_number'],
                      value_vars=['Hydrophobic', 'Hydrophilic'],
                      var_name='Contact type')
    return melt_df


def plot_data(df, label, ax):
    total_contacts = df.groupby(['Peptide_number'],
                                as_index=False).sum()
    hydrophobic = df[df['Contact type'] == 'Hydrophobic']
    sns.barplot(x='Peptide_number', y='value', data=total_contacts,
                ax=ax, palette=['lightsteelblue'],
                label='Hydrophilic')
    sns.barplot(x='Peptide_number', y='value', data=hydrophobic,
                ax=ax, palette=['thistle'],
                label='Hydrophobic')
...
if snakemake.params.k18_type == 'K18':
    labels = ['A1','R2','T3','K4','Q5','T6','A7','R8','K9','S10', 'T11',
                    'G12', 'G13', 'K14', 'A15', 'P16', 'R17', 'K18', 'Q19', 'L20',]
elif snakemake.params.k18_type == 'K18Ac':
    labels = ['A1','R2','T3','K4','Q5','T6','A7','R8','K9Me$_\mathregular{3}$',
              'S10', 'T11', 'G12', 'G13', 'K14', 'A15', 'P16', 'R17', 'K18Ac',
              'Q19', 'L20',]

fig, ax = plt.subplots(figsize=(8,5), constrained_layout=True)

plot_data(df, labels, ax=ax)
